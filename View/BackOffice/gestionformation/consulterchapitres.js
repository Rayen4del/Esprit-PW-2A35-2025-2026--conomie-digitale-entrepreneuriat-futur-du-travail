...
window.openView = function (id) {
    ...
    const formation_id = new URLSearchParams(window.location.search).get("formation_id");

    fetch(`searchchapitres.php?search=${value}&page=${page}&sort=${sort}&formation_id=${formation_id}`)
        .then(res => res.text())
        .then(data => {
