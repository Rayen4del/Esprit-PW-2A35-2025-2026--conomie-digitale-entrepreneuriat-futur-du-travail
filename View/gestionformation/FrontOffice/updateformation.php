...
if (!empty($_FILES['image']['name'])) {
    ...
    $folder = defined('UPLOADS_DIR') ? UPLOADS_DIR : __DIR__ . "/../../../uploads/";
    if (!is_dir($folder)) {
        mkdir($folder, 0777, true);
    }

    $newPath = (defined('UPLOADS_URL') ? UPLOADS_URL : 'uploads/') . $imgName;

    move_uploaded_file($tmp, $folder . $imgName);

    $oldFullPath = (defined('UPLOADS_DIR') ? UPLOADS_DIR : __DIR__ . "/../../../uploads/") . basename($imagePath);
